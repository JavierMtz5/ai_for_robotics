...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    omega = matrix([[0. for _ in range(2 * N + 2 * num_landmarks)] for _ in range(2 * N + 2 * num_landmarks)])
    xi = matrix([[0.] for _ in range(2 * N + 2 * num_landmarks)])

    # Set initial pose constraint
    omega.value[0][0], omega.value[1][1] = 1., 1.
    xi.value[0][0], xi.value[1][0] = 50., 50.

    # Set measurement and motion constraints
    for index, data_batch in enumerate(data):
        ...
        # Set motion constraint for x-axis
        omega.value[index_x_prev][index_x_prev] += 1. / motion_noise
        omega.value[index_x_post][index_x_prev] += -1 * (1. / motion_noise)
        omega.value[index_x_prev][index_x_post] += -1 * (1. / motion_noise)
        omega.value[index_x_post][index_x_post] += 1. / motion_noise

        xi.value[index_x_prev][0] += (1. / motion_noise) * -1 * dx
        xi.value[index_x_post][0] += (1. / motion_noise) * dx

        # Set motion constraint for y-axis
        omega.value[index_y_prev][index_y_prev] += 1. / motion_noise
        omega.value[index_y_post][index_y_prev] += -1 * (1. / motion_noise)
        omega.value[index_y_prev][index_y_post] += -1 * (1. / motion_noise)
        omega.value[index_y_post][index_y_post] += 1. / motion_noise

        xi.value[index_y_prev][0] += (1. / motion_noise) * -1 * dy
        xi.value[index_y_post][0] += (1. / motion_noise) * dy

        for measure in measurement:
            index_L_x = 2 * (N + measure[0])
            index_L_y = 2 * (N + measure[0]) + 1

            # Set measurement constraints for x-axis
            omega.value[index_x_prev][index_x_prev] += 1. / measurement_noise
            omega.value[index_x_prev][index_L_x] += -1 * (1. / measurement_noise)
            omega.value[index_L_x][index_x_prev] += -1 * (1. / measurement_noise)
            omega.value[index_L_x][index_L_x] += 1. / measurement_noise

            xi.value[index_x_prev][0] += -1 * (1. / measurement_noise) * measure[1]
            xi.value[index_L_x][0] += (1. / measurement_noise) * measure[1]

            # Set measurement constraints for x-axis

            omega.value[index_y_prev][index_y_prev] += 1. / measurement_noise
            omega.value[index_y_prev][index_L_y] += -1 * (1. / measurement_noise)
            omega.value[index_L_y][index_y_prev] += -1 * (1. / measurement_noise)
            omega.value[index_L_y][index_L_y] += 1. / measurement_noise

            xi.value[index_y_prev][0] += -1 * (1. / measurement_noise) * measure[2]
            xi.value[index_L_y][0] += (1. / measurement_noise) * measure[2]
