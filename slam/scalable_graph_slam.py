...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    omega = matrix([[0. for _ in range(2 * (num_landmarks + 1))] for _ in range(2 * (num_landmarks + 1))])
    xi = matrix([[0.] for _ in range(2 * (num_landmarks + 1))])

    # Set initial pose contraint
    omega.value[0][0] += 1
    omega.value[1][1] += 1
    ...
    for data_batch in data:
        ...
        # Add measurement contraints
        for measure in measurement:
            index_L_x = 2 + 2 * measure[0]
            index_L_y = 3 + 2 * measure[0]

            # Add measurement constraint for x-axis
            ...
            # Add measurement constraint for y-axis
        ...
        # Add motion constraints for x-axis
        ...
        # Add motion constraints for y-axis
...
def solution_check(result, answer_mu, answer_omega):
    if len(result) != 2:
        print
        "Your function must return TWO matrices, mu and Omega"
        return False
    ...
    if user_mu.dimx != answer_mu.dimx or user_mu.dimy != answer_mu.dimy:
        print
        "Your mu matrix doesn't have the correct dimensions. Mu should be a", answer_mu.dimx, " x ", answer_mu.dimy, "matrix."
        return False
    else:
        print
    ...
    if user_omega.dimx != answer_omega.dimx or user_omega.dimy != answer_omega.dimy:
        print
        "Your Omega matrix doesn't have the correct dimensions. Omega should be a", answer_omega.dimx, " x ", answer_omega.dimy, "matrix."
        return False
    else:
        print
